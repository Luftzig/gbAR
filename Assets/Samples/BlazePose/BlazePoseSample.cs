...
public sealed class BlazePoseSample : MonoBehaviour
{
    ...
    [SerializeField] RawImage debugView;
    ...
    void Start()
    {
        ...
        ARCameraManager arCameraManager = transform.parent.GetComponentInChildren<ARCameraManager>();
        Debug.Assert(arCameraManager != null);
        arCameraManager.frameReceived += OnFrameReceived;
    }

    private void OnFrameReceived(ARCameraFrameEventArgs frameEvent)
    {
        var texture = frameEvent.textures.First();
        webcamTexture = texture;
        // cameraView.texture = texture;
        // debugText.text = $"Texture: {texture.width}x{texture.height}";
    }
    ...
    void Update()
    {
        ...
        Debug.Log($"BlazePose: pose.score = {pose.score}");
        if (pose.score < 0)
        {
            return;
        }
        ...
        if (debugText != null)
        {
            Vector3 rightWrist = landmarkResult.joints[(int) Joints.JointRightWrist];
            Vector3 leftWrist = landmarkResult.joints[(int) Joints.JointLeftWrist];
            debugText.text = $"Wrists R: {rightWrist.ToString()}; L: {leftWrist.ToString()}";
        }
        else
        {
            Debug.LogError("No debug output found");
        }
    ...
    void DrawJoints(Vector3[] joints)
    {
        ...
        // Matrix4x4 mtx = WebCamUtil.GetMatrix(-webcamTexture.videoRotationAngle, false,
